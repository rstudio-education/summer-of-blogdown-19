...
1. In your `config.toml` file, update your `permalinks` as follows:
...
1. Use the new post add-in (see figure \@ref(fig:addin)) to create a new `.Rmd` post bundle- pick one of the academic theme's archetypes and run with it! (make triple-sure it is, in fact, *bundled*)

```{r addin, echo = FALSE, out.width="80%", fig.cap = "The new post add-in"}
knitr::include_graphics("../assets/kelly-newpost.png")
```

1. Add some R code to your post. Using a built-in dataset (bore us here), make a simple plot using your favorite plotting package (for tidyverse team members, bonus points for using [`ggplot2`](https://ggplot2.tidyverse.org/) + [`gganimate`](https://gganimate.com/)!)
...
1. In your new post, add a link to another section of your blog, using *relative links*.
